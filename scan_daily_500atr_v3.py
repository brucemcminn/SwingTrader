...
# History file
OUT_FILE = Path("daily_long_candidates_top500atr.csv")

# Get symbols from pre-defined list
def get_symbols() -> list[str]:
    df = pd.read_csv("atr_top500.csv")
    return df["SYMBOL"].astype(str).str.strip().tolist()

# Get OHLCV from Yahoo finance and normalize
...
def main():
    symbols = get_symbols()
    print(f"Scanning {len(symbols)} top 500 symbols using ATR...")
